...
# DO NOT EDIT
    ...
    if test "$?" = "0"; then
        ...
        if false; then :
			...
			7 "Actisys Act220L(+) SIR driver support" 2> $TEMP
            if test "$?" = "0"; then
                {
                set `cat $TEMP`
                if false; then :
			...
			0 "DViCO USB Remote" \
			a "Home Electronics Tira USB device" \
			b "Igor Cesko's USB IR Receiver" \
			c "Iguanaworks USB IR Transceiver" \
			d "IRLink VS USB IR Receiver" \
			e "PCMAK USB receiver" \
			f "Remotec Multimedia PC Remote BW6130" \
			g "Samsung USB IR Receiver" \
			h "Sasem OnAir Remocon-V" \
			i "Sound Blaster Extigy USB sound card (exaudio)" \
			j "Sound Blaster Extigy/Audigy 2 NX (ALSA snd-usb-audio)" \
			k "Soundgraph iMON 2.4G DT & LT" \
			l "Soundgraph iMON MultiMedian IR/VFD" \
			m "Soundgraph iMON IR/LCD" \
			n "Soundgraph iMON Knob" \
			o "Soundgraph iMON PAD IR/VFD" \
			p "Soundgraph iMON RSC" \
			q "Streamzap PC Remote" \
			r "TechnoTrend USB IR receiver" \
			s "USB-UIRT" \
			t "VLSystem MPlay Blast" \
			u "Windows Media Center Remotes (old version, MicroSoft USB ID)" \
			v "Windows Media Center Remotes (new version, Philips et al.)" 2> $TEMP
            if test "$?" = "0"; then
                {
                set `cat $TEMP`
                if false; then :
                ...
                elif test "$1" = "a"; then LIRC_DRIVER=tira;		DRIVER_PARAMETER=ttyUSB1; DRIVER_PARAM_TYPE=ttyUSB;
                elif test "$1" = "b"; then LIRC_DRIVER=igorplugusb;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "c"; then LIRC_DRIVER=iguanaIR;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "d"; then LIRC_DRIVER=irlink;		DRIVER_PARAMETER=ttyUSB1; DRIVER_PARAM_TYPE=ttyUSB;
                elif test "$1" = "e"; then LIRC_DRIVER=pcmak_usb;	DRIVER_PARAMETER=ttyUSB1; DRIVER_PARAM_TYPE=ttyUSB;
                elif test "$1" = "f"; then LIRC_DRIVER=bw6130;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "g"; then LIRC_DRIVER=samsung;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "h"; then LIRC_DRIVER=sasem;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "i"; then LIRC_DRIVER=exaudio;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "j"; then LIRC_DRIVER=alsa_usb;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "k"; then LIRC_DRIVER=imon_24g;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "l"; then LIRC_DRIVER=imon;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "m"; then LIRC_DRIVER=imon_lcd;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "n"; then LIRC_DRIVER=imon_knob;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "o"; then LIRC_DRIVER=imon_pad;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "p"; then LIRC_DRIVER=imon_rsc;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "q"; then LIRC_DRIVER=streamzap;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "r"; then LIRC_DRIVER=ttusbir;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "s"; then LIRC_DRIVER=usb_uirt_raw;	DRIVER_PARAMETER=ttyUSB1; DRIVER_PARAM_TYPE=ttyUSB;
                elif test "$1" = "t"; then LIRC_DRIVER=mplay;		DRIVER_PARAMETER=ttyUSB1; DRIVER_PARAM_TYPE=ttyUSB;
                elif test "$1" = "u"; then LIRC_DRIVER=mceusb;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "v"; then LIRC_DRIVER=mceusb2;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                fi
                }
                else
                    return;
